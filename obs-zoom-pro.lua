...
function StateMachine:complete()
    if self.state == ZoomState.ZOOMING_IN
       ...
       or self.state == ZoomState.JUMPING then
        ...
        self.zoom_time = 0
        log("State: " .. self.state .. " → ZOOMED_IN (complete)")
    elseif self.state == ZoomState.ZOOMING_OUT then
        ...
        self.zoom_time = 0
        log("State: ZOOMING_OUT → IDLE (complete)")
    end
end
...
function SceneMemory:set(scene_name, source_name)
    ...
    log("SceneMemory: " .. scene_name .. " → " .. source_name)
    return true
end
    ...
    if self.zoom_blur_enabled and self.zoom_blur_filter then
        ...
        if state_machine:is_animating() and
           (state_machine.state == ZoomState.ZOOMING_IN or
            ...
            -- Bell curve: 0 at start → peak at 50% → 0 at end
...
function script_update(data)
    ...
    debug_logs = data.debug_logging or false
    settings.debug_logging = debug_logs
    settings.easing_type = data.easing_type or defaults.easing_type
    settings.smooth_time = data.smooth_time or defaults.smooth_time
    settings.max_speed = data.max_speed or defaults.max_speed
...
function script_properties()
    ...
    obs.obs_properties_add_group(props, "api_server", "🌐 UDP API Server", obs.OBS_GROUP_NORMAL, grp_api)
    ...
    obs.obs_properties_add_text(grp_help, "help_text",
        ...
        "   • 'Toggle Mouse Follow' — Enable/disable follow\n" ..
        "3. Press the hotkey to zoom!\n\n" ..
        "TIPS:\n" ..
        "• Zoom centers on mouse position\n" ..
        "• Enable Auto-Follow for smooth tracking\n" ..
        "• Adjust Dead Zone to reduce jitter",
        obs.OBS_TEXT_INFO)
